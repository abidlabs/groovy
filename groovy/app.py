...
from groovy.types import Image, String, StepMessage, raw_value_to_step_message
    ...
    with gr.Blocks(theme=theme) as app:
        ...
        def run_flow(prompt):
            ...
            for step_message in streamer(prompt):
                ...
                if isinstance(step_message, String):
                    log.append(gr.ChatMessage(content=step_message, role="assistant"))
                elif isinstance(step_message, Image):
                    log.append(gr.ChatMessage(content=gr.Image(step_message), role="assistant"))

                    if step_message.save_in_recording and save_recording:
                        img_with_text = add_step_counter(step_message.value, len(images_for_gif) + 1)
                    ...
                    raise ValueError(
                        f"Cannot handle type: {type(step_message)} generated by streamer function."
                    )
                yield log
